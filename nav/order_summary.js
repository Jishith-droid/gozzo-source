...
async function submitAddrForm(event) {
  ...
  if (user) {
    try {
      ...
      // Wait for all database operations to complete before proceeding
      ...
      const addressSnapshot = await get(ref(db, `users/${userId}/address`));
...
async function generateInvoice(uid, productCodes, orderid, productQuantities, products) {
  ...
  let selectedPacks = JSON.parse(sessionStorage.getItem('selectedProductPacks')) || {};
  let expDate;
  const comName = 'PotItUp';
  const comEmail = 'potitupspprt@gmail.com';
  ...
  <div class="nameAndLogo">
       <div class="company-logo">
          <img src="https://cdn.jsdelivr.net/gh/Jishith-MP/test@main/logo%20(1).png" alt="Company Logo">
       </div>
    <h2 class="company-name">PotItUp</h2>
   </div>
